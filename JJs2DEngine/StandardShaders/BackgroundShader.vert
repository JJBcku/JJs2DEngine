...
void main()
{
	vec2 verticeOffset = vertexData[vertexIndexes[gl_VertexIndex]];
	vec2 textureOffset = textureData[vertexIndexes[gl_VertexIndex]];

	gl_Position = vec4(verticeOffset, 1.0, 1.0);
	
	vec2 texCoord = inTexCoord + (inTexSize * textureOffset);
