...
namespace JJs2DEngine
{
	...
	void UiVertexDataLayerVersionInternal::ChangeObjectsTexture(bool inPreloadedTexturesList, std::pair<size_t, size_t> newTextureID, size_t objectsIndex)
	{
		if (objectsIndex >= _objectList.size())
			throw std::runtime_error("UiVertexDataLayerVersionInternal::ChangeObjectsTexture Error: Program tried to change non-existent object!");

		if (!_objectList[objectsIndex].has_value())
			throw std::runtime_error("UiVertexDataLayerVersionInternal::ChangeObjectsTexture Error: Program tried to change unused object!");
